...
class MayaBatchPlugin (DeadlinePlugin):
    ...
    def StartJob( self ):
        ...
        if self.Version >= 2016.5:
            useLegacyRenderLayers = self.GetIntegerPluginInfoEntryWithDefault( "UseLegacyRenderLayers", 0 )
    ...
    def GetStartFrameCommand( self ):
        if len( self.StartFrame ) > 0:
            if self.Renderer == "file" or ( self.UsingRenderLayers and len( self.RenderLayer ) == 0 ):
                ...
                return 'removeRenderLayerAdjustmentAndUnlock .animation; setAttr .animation 1; removeRenderLayerAdjustmentAndUnlock .startFrame; setAttr .startFrame ' + self.StartFrame + ';'
            elif self.Renderer == "mayavector" or self.Renderer == "mayahardware" or self.Renderer == "mayahardware2" or self.Renderer == "turtle" or self.Renderer == "arnold" or self.Renderer == "arnoldexport" or self.Renderer == "redshift" or self.Renderer == "vray" or self.Renderer == "vrayexport" or self.Renderer == "ifmirayphotoreal":
    ...
    def GetRenumberFrameStartCommand( self ):
        if len( self.RenumberFrameStart ) > 0:
            if self.Renderer == "file" or ( self.UsingRenderLayers and len( self.RenderLayer ) == 0 ):
                ...
                return 'removeRenderLayerAdjustmentAndUnlock .modifyExtension; setAttr .modifyExtension 1; removeRenderLayerAdjustmentAndUnlock .startExtension; setAttr .startExtension ' + self.RenumberFrameStart + ';'
            elif self.Renderer == "mayavector" or self.Renderer == "mayahardware" or self.Renderer == "mayahardware2" or self.Renderer == "turtle" or self.Renderer == "arnold" or self.Renderer == "arnoldexport" or self.Renderer == "redshift" or self.Renderer == "ifmirayphotoreal" :
    ...
    def GetMiscCommands( self ):
        if self.Renderer == "maxwell" or self.Renderer == "maxwellexport":
            ...
            if self.OverrideGpuAffinity():
                ...
                if gpusPerTask == 0 and gpusSelectDevices != "":
                    ...
                    if len(notFoundGPUs) > 0:
                        self.LogWarning( "The slave is overriding it's GPU affinity and the following GPUs do not match the slaves affinity so they will not be used: " + ",".join(notFoundGPUs) )
                    if len(foundGPUs) == 0:
                        self.FailRender( "The slave does not have affinity for any of the GPUs specified in the job." )      
                elif gpusPerTask > 0:
                    if gpusPerTask > len( overrideGPUs ):
                        self.LogWarning( "The slave is overriding it's GPU affinity and the slave only has affinity for " + str( len( overrideGPUs ) ) + " slaves of the " + str( gpusPerTask ) + " requested." )
                ...
                gpus = ""
                for gpu in resultGPUs:
                    if not gpus == "":
                        gpus += ","
                    gpus = gpus + str( gpu )
                
                self.LogInfo( "This slave is overriding it's GPU affinity, so the following GPUs will be used by RedShift: " + gpus )
                ...
                return 'redshiftSelectCudaDevices({' + gpusSelectDevices + '});'
                ...
                gpus = ",".join(gpuList)
                self.LogInfo( "GPUs per task is greater than 0, so the following GPUs will be used: " + gpus )
                return 'redshiftSelectCudaDevices({' + gpus + '});'
            ...
            # If the number of gpus per task is set, then need to calculate the gpus to use.
            gpusPerTask = self.GetIntegerPluginInfoEntryWithDefault( "IRayGPUsPerTask", 0 )
            gpusSelectDevices = self.GetPluginInfoEntryWithDefault( "IRayGPUsSelectDevices", "" )
            ...
            if self.OverrideGpuAffinity():
            ...
            elif gpusPerTask > 0 or gpusSelectDevices != "":
                ...
                if gpusPerTask == 0 and gpusSelectDevices != "":              
                    separateGpus = gpusSelectDevices.split(",")
                    
                    for gpu in separateGpus:
                        intGpu = int(gpu)
                        miscBuilder.AppendLine( "ifmResource -e "+str(intGpu +1 )+';' )
                    ...
                    gpuList = []
                    for i in range((self.GetThreadNumber() * gpusPerTask), (self.GetThreadNumber() * gpusPerTask) + gpusPerTask):
                        miscBuilder.AppendLine( "ifmResource -e "+str(i+1)+';'   )
                        gpuList.append(str(i))
    ...
    def GetRenderCommand( self ):
        if self.Renderer == "file" or (self.UsingRenderLayers and len( self.RenderLayer ) == 0 ):
            ...
            return 'setMayaSoftwareLayers($rl, $rp); setImageSizePercent($resize); redshiftBatchRender("");'
        elif self.Renderer == "vray":
            ...
            if len( mxsFile ) > 0:
                maxwellBuilder.AppendLine( 'setAttr -type "string" maxwellRenderOptions.mxsPath "' + mxsFile.replace( "\\", "/" ) + '";' )
...
class MayaBatchProcess (ManagedProcess):
    ...
    def InitializeProcess( self ):
        ...
        self.AddStdoutHandlerCallback( ".*multishot finished.*").HandleCallback += self.HandleMultishotFinish
    ...
    def HandleMultishotFinish( self ):
        self.deadlinePlugin.LogWarning("Multishot Exiting with Success :)")
        self.deadlinePlugin.ExitWithSuccess()
